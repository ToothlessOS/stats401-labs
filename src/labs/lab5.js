...
async function draw() {
    ...
    const matFillScale = 
        d3.scaleSequential()
        .domain(
            [0, d3.max(links, d => d.travel_time_min)]
        )
    ...
    const cell = matrixGroup
        ...
        .attr(
            "fill",
            d => d.weight > 0 ? matFillScale(d.weight) : "#f3f3f3"
        );

    // Matrix-cell tooltip: reuse the graph tooltip helpers above — they
    // populate the shared #tooltip div, which is enough since only one
    // thing is hovered at a time. A matrix cell stands for a station pair
    // (row ↔ col), so resolve the ids back to stations via stationById.
    const matCellTooltipHtml = (d) => {
        const from = stationById.get(d.row);
        const to = stationById.get(d.col);
        if (d.weight > 0) {
            return `<strong>${from.station_name} (${from.district} district; ${from.station_type})<br>
                <=> <br>
                ${to.station_name}(${to.district} district; ${to.station_type})</strong><br> ` +
                `Route type: ${d.type}<br>` +
                `Travel time: ${d.weight} min`;
        }
        return `<strong>${from.station_name} X ${to.station_name}</strong><br>` +
            `No direct route`;
    };

    cell.on("mouseover", (event, d) => showTooltip(event, matCellTooltipHtml(d)));
    cell.on("mousemove", moveTooltip);
    cell.on("mouseout", hideTooltip);

    // Color legend for the matrix: a vertical gradient bar in the empty
    // right margin (the plot spans x 100–900, the SVG is 1000 wide). It
    // reuses matFillScale's own interpolator so the bar colors always
    // match the cell fills: low travel times are pale, high ones dark.
    const barX = 915;
    const barW = 14;
    const barTop = 100;
    const barBottom = 900;
    ...
    const barScale = d3.scaleLinear()
        .domain(matFillScale.domain())
        .range([barBottom, barTop]);   // high value at the top

    const travelGrad = matrixSvg.append("defs")
        .append("linearGradient")
        .attr("id", "matrix-travel-grad")
        .attr("gradientUnits", "userSpaceOnUse")
        .attr("x1", 0)
        .attr("x2", 0)
        .attr("y1", barBottom)         // offset 0 -> light orange
        .attr("y2", barTop);           // offset 1 -> dark orange

    // interpolateOranges is a multi-stop ramp, not a straight RGB line
    // between its endpoints, so sample the scale at many values and let
    // the browser fill linearly only between adjacent samples. Each stop
    // then carries exactly the color matFillScale gives the same value.
    const colorStops = 40;
    const [minTime, maxTime] = matFillScale.domain();

    travelGrad.selectAll("stop")
        .data(d3.range(colorStops))
        .join("stop")
        .attr("offset", i => i / (colorStops - 1))
        .attr(
            "stop-color",
            i => matFillScale(minTime + (i / (colorStops - 1)) * (maxTime - minTime))
        );
    ...
    matrixSvg.append("rect")
        ...
        .attr("fill", "url(#matrix-travel-grad)")
        .attr("stroke", "#bbb")
        .attr("stroke-width", 0.5);

    matrixSvg.append("g")
        .attr("transform", `translate(${barX + barW + 8}, 0)`)
        .call(
